...
const HandlePermissions: React.FC<OwnProps> = ({children}) => {
    ...
    console.log(PERMISSIONS[currentUserAccessType]?.find(page => location.pathname.includes(page)) )

    return PERMISSIONS[currentUserAccessType]?.find(page => location.pathname.includes(page)) ? children : <Unauthorized />
}
