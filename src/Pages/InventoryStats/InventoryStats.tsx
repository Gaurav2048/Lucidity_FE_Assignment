...
const InventoryStats: React.FC = () => {
    ...
    const CardDetails = useMemo<Array<CardType>>(() => {
        ...
        const totalStoreValue = inventory.reduce((sum, { value }) => sum + currencyConverter(value), 0)
        const outOfStock = inventory.filter(item => item.quantity === NO_ITEM_IN_STORE).length
    ...
    return <div className="flex gap-4 p-8">
        {CardDetails.map(info => <AppCard title={info.title} icon={info.icon} value={info.value} key={info.title} />)}
    </div>
}
